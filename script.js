// ★★★ 初期値変更、統計追加、ボタンサイズ調整反映版 (省略なし) ★★★
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOMContentLoaded: イベント発火");

    // --- DOM要素の取得 (ボーナスカウント表示用を追加) ---
    ...
    const longWordCountSpan = document.getElementById('longWordCount'); // ★ Long Word 回数
    const veryLongWordCountSpan = document.getElementById('veryLongWordCount'); // ★ Very Long Word 回数
    const closeStatsButton = document.getElementById('closeStatsButton');
    const restartButton = document.getElementById('restartButton');
    const overlay = document.getElementById('overlay');

    // 必須要素チェック (ボーナスカウントも追加)
    if (!initialSetupDiv || !gameWrapper || !startButton || !inputArea || !keyboardDiv || !confirmButton || !logListUl || !passButton || !passCountSpan || !totalPointsSpan || !endGameButton || !statsPopup || !finalPointsSpan || !finalPassCountSpan || !closeStatsButton || !overlay || !restartButton || !longestWordTextSpan || !longestWordLengthSpan || !zeroCharsListSpan || !longWordCountSpan || !veryLongWordCountSpan ) {
    ...
    let firstZeroChars = [];
    let longWordCount = 0; // ★ Long Word 回数カウンター
    let veryLongWordCount = 0; // ★ Very Long Word 回数カウンター
    ...
    // ポイント変動ポップアップ表示
    function displayPointChangePopup(change, bonusType = null) {
        if (!pointChangePopup || !popupBonusText || !popupPointValue) { console.warn("ポップアップ要素見つからず"); return; }
        let pointText = ""; let bonusMessage = ""; pointChangePopup.classList.remove('plus', 'minus', 'show', 'has-bonus');
        if (change > 0) { pointText = `+${change}`; pointChangePopup.classList.add('plus'); }
        else if (change < 0) { pointText = `${change}`; pointChangePopup.classList.add('minus'); }
        else { return; }
        if (bonusType === "LONG") { bonusMessage = "LONG WORD !"; pointChangePopup.classList.add('has-bonus'); }
        else if (bonusType === "VERY LONG") { bonusMessage = "VERY LONG WORD !!"; pointChangePopup.classList.add('has-bonus'); }
        popupBonusText.textContent = bonusMessage; popupPointValue.textContent = pointText;
        requestAnimationFrame(() => { pointChangePopup.classList.add('show'); });
        if (pointChangeTimeout) { clearTimeout(pointChangeTimeout); }
        pointChangeTimeout = setTimeout(() => { pointChangePopup.classList.remove('show'); }, 1700);
    } // displayPointChangePopup 関数の閉じ括弧

    // 確定ボタン処理 (ボーナスカウント処理追加)
    function handleConfirm() {
        const word = inputArea.value;
        console.log("handleConfirm: 実行, word=", word);
        if (word.length > 0) {
            const wordLength = word.length;
            let bonusPoints = 0; let bonusType = null;
            // ボーナス判定とカウント
            if (wordLength >= 10) {
                bonusPoints = 2; bonusType = "VERY LONG";
                veryLongWordCount++; // ★ Very Long カウントアップ
                console.log("  Very Long Word! 回数:", veryLongWordCount);
            } else if (wordLength >= 7) {
                bonusPoints = 1; bonusType = "LONG";
                longWordCount++; // ★ Long カウントアップ
                console.log("  Long Word! 回数:", longWordCount);
            }
    ...
    function addLogEntry(text, isPass) {
        const firstLogItem = logListUl.firstChild; if (firstLogItem) { const oldButton = firstLogItem.querySelector('.cancel-log-button'); if (oldButton) { oldButton.classList.add('hidden'); } }
        const listItem = document.createElement('li'); const wordSpan = document.createElement('span'); wordSpan.classList.add('log-word'); wordSpan.textContent = text; listItem.appendChild(wordSpan);
        const pointSpan = document.createElement('span'); pointSpan.classList.add('log-point'); const cancelButton = document.createElement('button'); cancelButton.textContent = '取消'; cancelButton.classList.add('cancel-log-button'); cancelButton.addEventListener('click', handleCancelLog);
        if (isPass) { pointSpan.textContent = '(-1)'; pointSpan.classList.add('minus'); cancelButton.dataset.isPass = 'true'; cancelButton.dataset.word = ''; wordSpan.appendChild(pointSpan); }
        else { const wordLength = text.length; let bonusPoints = 0; if (wordLength >= 10) { bonusPoints = 2; } else if (wordLength >= 7) { bonusPoints = 1; } const basePointSpan = document.createElement('span'); basePointSpan.classList.add('log-point', 'plus'); basePointSpan.textContent = `(+${wordLength})`; wordSpan.appendChild(basePointSpan); if (bonusPoints > 0) { const bonusPointSpan = document.createElement('span'); bonusPointSpan.classList.add('log-point', 'bonus'); bonusPointSpan.textContent = `(+${bonusPoints})`; wordSpan.appendChild(bonusPointSpan); } cancelButton.dataset.isPass = 'false'; cancelButton.dataset.word = text; }
        listItem.appendChild(cancelButton); logListUl.insertBefore(listItem, logListUl.firstChild);
    } // addLogEntry 関数の閉じ括弧

    // ログ取り消し処理 (ボーナスカウント減算追加)
    function handleCancelLog(event) {
        ...
        if (isPass) { if (passCounter > 0) { passCounter--; passCountSpan.textContent = passCounter; totalPoints++; console.log("  パス取消、カウンター:", passCounter, "ポイント:", totalPoints); if (totalPointsSpan) { totalPointsSpan.textContent = totalPoints; } else { console.error("Error: totalPointsSpan not found!"); } } }
            ...
            if (wordToCancel) {
                const wordLength = wordToCancel.length; let bonusPoints = 0;
                // ボーナス判定とカウント減算
                if (wordLength >= 10) {
                    bonusPoints = 2;
                    if (veryLongWordCount > 0) veryLongWordCount--;
                    console.log("  Very Long 取り消し、回数:", veryLongWordCount);
                } else if (wordLength >= 7) {
                    bonusPoints = 1;
                    if (longWordCount > 0) longWordCount--;
                    console.log("  Long 取り消し、回数:", longWordCount);
                }
                const pointChange = wordLength + bonusPoints;

                console.log("  単語取り消し, wordLength=", wordLength, "bonus=", bonusPoints);
                for (const char of wordToCancel) { if (allValidCharsList.includes(char)) { const shouldRemoveFromZeroList = (charCounts[char] === 0); charCounts[char]++; updateKeyDisplay(char); if (shouldRemoveFromZeroList) { const index = firstZeroChars.indexOf(char); if (index > -1) { firstZeroChars.splice(index, 1); console.log("枯渇リストから削除:", char); } } } }
        ...
        logListUl.removeChild(listItem); const newFirstLogItem = logListUl.firstChild; if (newFirstLogItem) { const newButton = newFirstLogItem.querySelector('.cancel-log-button'); if (newButton) { newButton.classList.remove('hidden'); console.log("  新先頭ボタン表示"); } }
    ...
    function handleEndGame() {
        if (!confirm("本当にゲームを終了しますか？")) { return; }
        console.log("ゲーム終了処理 開始");
        let longestWord = "-"; let longestLength = 0; const logItems = logListUl.querySelectorAll('li:not(.pass-log)'); logItems.forEach(item => { const button = item.querySelector('.cancel-log-button'); if (button && button.dataset.word) { const word = button.dataset.word; if (word.length > longestLength) { longestLength = word.length; longestWord = word; } } });
        const zeroCharsText = firstZeroChars.slice(0, 3).join(', ') || "-";

        // 統計情報表示
        finalPointsSpan.textContent = totalPoints;
        finalPassCountSpan.textContent = passCounter;
        longestWordTextSpan.textContent = longestWord;
        longestWordLengthSpan.textContent = longestLength;
        longWordCountSpan.textContent = longWordCount; // ★ Long Word 回数表示
        veryLongWordCountSpan.textContent = veryLongWordCount; // ★ Very Long Word 回数表示
        zeroCharsListSpan.textContent = zeroCharsText;

        statsPopup.style.display = 'block';
        overlay.style.display = 'block';
        console.log("ゲーム終了処理 完了");
    } // handleEndGame 関数の閉じ括弧
    ...
    // ゲーム開始処理 (ボーナスカウンターリセット追加)
    function startGame() {
        console.log("startGame: 開始");
        try {
            const countValue = initialCountInput ? initialCountInput.value : "3"; // ★ 初期値反映
            const count = parseInt(countValue, 10);
            console.log("  パース後カウント:", count);
            if (isNaN(count) || count < 1) { alert('正しい初期個数を入力してください（1以上）。'); console.warn("startGame: 不正カウント中断"); return; }
            console.log("  charCounts初期化開始"); allValidCharsList.forEach(char => { charCounts[char] = count; }); console.log("  charCounts初期化完了");
            console.log("  カウンターと統計リセット開始");
            passCounter = 0; if (passCountSpan) passCountSpan.textContent = '0'; else console.error("Error: passCountSpan not found!");
            totalPoints = 0; if (totalPointsSpan) totalPointsSpan.textContent = '0'; else console.error("Error: totalPointsSpan not found!");
            firstZeroChars = [];
            longWordCount = 0; // ★ リセット
            veryLongWordCount = 0; // ★ リセット
            if (pointChangePopup) pointChangePopup.classList.remove('show');
            console.log("  カウンターと統計リセット完了");
            ...
            console.error("startGameエラー:", error);
            alert("ゲーム開始エラー");
        }
    } // startGame 関数の閉じ括弧
